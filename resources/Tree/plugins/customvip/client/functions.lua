...
function Tree.Func_.CoinsToRound(coins)
    local MAX = 3000
    local MIN = 100
    ...
    if coins > MAX then
        ...
        local remainder = coins % MIN
        if remainder >= MIN / 2 then
            return math.floor(coins - remainder + MIN)
        else
            return math.floor(coins - remainder)
        end
    end
end


function Tree.Func_.GetMitrailVeh(TableCustoms)
...
function Tree.Func_.SetVehicleProperties(vehicle, props)
