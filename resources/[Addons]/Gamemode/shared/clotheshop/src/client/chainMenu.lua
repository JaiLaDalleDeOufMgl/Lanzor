...
function _Client.open:chainMenu()
    ...
    while chainMenu do
        ...
        RageUIClothes.IsVisible(chainMenu, function()
            ...
            RageUIClothes.Line()
            RageUIClothes.List('Chaine 1', Chaine1, filter.accessories.Chaine1, desc, {}, true, {
                onActive = function() Uclothes:OnRenderCam() end, 
                onListChange = function(Index, Item) 
                    filter.accessories.Chaine1 = Index 
                    filter.accessories.Chaine2 = 1 
                    TriggerEvent('skinchanger:change', 'chain_1', filter.accessories.Chaine1 - 1) 
                end, 
                onSelected = function() 
                    local result = KeyboardInput('Indiquez le numéro que vous souhaitez sélectionner :','', '', 20) 
                    
                    if result ~= "" and tonumber(result) ~= nil then 
                        filter.accessories.Chaine1 = result 
                        TriggerEvent('skinchanger:change', 'chain_1', filter.accessories.Chaine1 - 1) 
                    else 
                        ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color..'Il semblerait que vous n\'avez entré aucune valeur. Assurez vous qu\'il s\'agisse bel et bien d\'un chiffre/nombre.') 
                    end 
                end
            })
    
            RageUIClothes.List('Chaine 2', Chaine2, filter.accessories.Chaine2, desc, {}, true, { 
                onActive = function() Uclothes:OnRenderCam() end, 
                onListChange = function(Index, Item) 
                    filter.accessories.Chaine2 = Index 
                    TriggerEvent('skinchanger:change', 'chain_2', filter.accessories.Chaine2 - 1) 
                end, 
                onSelected = function() 
                    local result = KeyboardInput('Indiquez le numéro que vous souhaitez sélectionner :','', '', 20); 
                    if result ~= "" and tonumber(result) ~= nil then 
                        filter.accessories.Chaine2 = result 
                        TriggerEvent('skinchanger:change', 'chain_2', filter.accessories.Chaine2 - 1) 
                    else 
                        ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color..'Il semblerait que vous n\'avez entré aucune valeur. Assurez vous qu\'il s\'agisse bel et bien d\'un chiffre/nombre.') 
                    end 
