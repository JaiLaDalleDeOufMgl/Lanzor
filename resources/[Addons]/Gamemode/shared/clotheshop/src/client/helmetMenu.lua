...
function _Client.open:helmetMenu()
    ...
    while helmetMenu do
        ...
        RageUIClothes.IsVisible(helmetMenu, function()
            ...
            RageUIClothes.Line()
            RageUIClothes.List('Casque 1', Chapeau1, filter.accessories.Chapeau1, desc, {}, true, { onActive = function() Uclothes:OnRenderCam() end, onListChange = function(Index, Item) filter.accessories.Chapeau1 = Index filter.accessories.Chapeau2 = 1 TriggerEvent('skinchanger:change', 'helmet_1', filter.accessories.Chapeau1 - 1) end, onSelected = function() local result = KeyboardInput('Indiquez le numéro que vous souhaitez sélectionner :','','', 20) if result ~= "" and tonumber(result) ~= nil then filter.accessories.Chapeau1 = result TriggerEvent('skinchanger:change', 'helmet_1', filter.accessories.Chapeau1 - 1) else ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color..'Il semblerait que vous n\'avez entré aucune valeur. Assurez vous qu\'il s\'agisse bel et bien d\'un chiffre/nombre.') end end})
            RageUIClothes.List('Casque 2', Chapeau2, filter.accessories.Chapeau2, desc, {}, true, { onActive = function() Uclothes:OnRenderCam() end, onListChange = function(Index, Item) filter.accessories.Chapeau2 = Index TriggerEvent('skinchanger:change', 'helmet_2', filter.accessories.Chapeau2 - 1) end, onSelected = function() local result = KeyboardInput('Indiquez le numéro que vous souhaitez sélectionner :','','', 20) if result ~= "" and tonumber(result) ~= nil then filter.accessories.Chapeau2 = result TriggerEvent('skinchanger:change', 'helmet_2', filter.accessories.Chapeau2 - 1) else ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color..'Il semblerait que vous n\'avez entré aucune valeur. Assurez vous qu\'il s\'agisse bel et bien d\'un chiffre/nombre.') end end})
