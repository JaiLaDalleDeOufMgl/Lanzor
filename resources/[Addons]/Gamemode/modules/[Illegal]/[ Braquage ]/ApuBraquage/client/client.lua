                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                wait = 1
                                DrawText3DApu(GetEntityCoords(bag).x, GetEntityCoords(bag).y, GetEntityCoords(bag).z, "Appuyez sur ["..exports.Tree:serveurConfig().Serveur.color.."X~s~] pour ~s~ramasser le sac~s~.", 12)
...
local function MenaceApuMore(apu)
    ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color.."Vous devez menacer APU.")
...
AddEventHandler("initializePoliceBlip2", function(ApuIndex)
    CreateThread(function()
        ...
        ESX.ShowNotification('~g'..exports.Tree:serveurConfig().Serveur.color..'s~ Accepter '..exports.Tree:serveurConfig().Serveur.color..'X~s~ Refuser')
        while true do
            Wait(0)
                        ...
                        if not eSup.IsRobbing then
                            ...
                            if canRob == true then
                                ...
                                if chance >= 0 and chance <= 25 then
                                    ...
                                    eSup.BarUpdate = AddTimerBar("Menace :",{percentage = 0.0, bg = {217, 255, 0}, fg = {255,250,0,255}})
                                    ...
                                    if GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(eSup.Peds[i]), true) <= 7.5 then
                                        if not IsPedDeadOrDying(eSup.Peds[i]) then
                                            ...
                                            if BraqueApuYesOrNo then
                                                ...
                                                ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color.."APU se fait déjà braquer")
                                            end
                                        end
                                    else
                                        ClearPedTasks(eSup.Peds[i])
                                ...
                                ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color.."Cette superette a déjà été braqué, le vendeur n'a plus rien.")
                            end
                            -- elseif canRob == 'no_cops' then
                            --     local wait = GetGameTimer() + 5000
                            --     while wait >= GetGameTimer() do
                            --         Wait(0)
                            --         ESX.ShowNotification(''..exports.Tree:serveurConfig().Serveur.color..'Gruppe6 viens de passer, les caisses sont vides.')
