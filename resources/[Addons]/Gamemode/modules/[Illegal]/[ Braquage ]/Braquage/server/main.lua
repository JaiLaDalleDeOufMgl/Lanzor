...
AddEventHandler('esx_holdupbank:rob', function(robb)
	...
	if Banks[robb] then
		...
		if not rob then
				...
				SetTimeout(300000, function()
					if robbers[_source] then
						...
						if xPlayer then
							...
							local webhookLink = exports.Tree:serveurConfig().Logs.BraquageBanque
			...
			TriggerClientEvent('esx:showNotification', _source, exports.Tree:serveurConfig().Serveur.color.."Un braquage est déjà en cours.")
		end
	end
end)
	...
	if Stores[robb] then
		...
		if rob == false then
			...
			for i=1, #xPlayers, 1 do
				local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
				if xPlayer.job.name == 'police' or xPlayer.job.name == 'bcso' then
					TriggerClientEvent('esx:showNotification', xPlayers[i], exports.Tree:serveurConfig().Serveur.color.." Braquage en cours à: "..exports.Tree:serveurConfig().Serveur.color.."" .. store.nameofstore)
			...
			TriggerClientEvent('esx:showNotification', source, exports.Tree:serveurConfig().Serveur.color.."Un braquage est déjà en cours.")
		end
	end
end)
