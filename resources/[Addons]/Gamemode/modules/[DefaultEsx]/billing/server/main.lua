			...
			if xPlayer.job.name == sharedAccountName then
				MySQL.Async.fetchAll('SELECT * FROM billing WHERE identifier = @a', {
					...
					MySQL.Async.execute('INSERT INTO billing (identifier, sender, target_type, target, label, amount) VALUES (@identifier, @sender, @target_type, @target, @label, @amount)',
					{
						...
						TriggerClientEvent('esx:showNotification', xTarget.source, "vous avez "..exports.Tree:serveurConfig().Serveur.color.."reçu~s~ une facture")

						local link = exports.Tree:serveurConfig().Logs.FactureLogs
			...
			if xTarget ~= nil then
				...
				if HaveCount and HaveCount.money >= amount then
					...
					MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
						...
						TriggerClientEvent('GamemodePass:addtaskcount:standart', xPlayer.source, 6, 1)
						ESX.AddSocietyMoney(target, amount);
						TriggerClientEvent('esx:showNotification', xPlayer.source, "vous avez ~g~payé~s~ une facture de "..exports.Tree:serveurConfig().Serveur.color..""..ESX.Math.GroupDigits(amount))
...
function sendWebhookforbill(name,message,color)
    local local_date = os.date('%H:%M:%S', os.time())
    local DiscordWebHook = exports.Tree:serveurConfig().Logs.FactureLogs
  ...
  local embeds = {
      {
          ...
          ["color"] =color,
          ["footer"]=  {
          ["text"]= "Powered by Gamemode ©   |  "..local_date.."",
         },
      }
  }
