...
username = exports.Tree:serveurConfig().Serveur.label						        -- Bot Username
avatar = "https://media.discordapp.net/attachments/855797269190017074/901249988942766131/logo_Gamemode.png"				-- Bot Avatar
communtiyName = "Azuryy"					            -- Icon top of the Embed
communtiyLogo = "https://media.discordapp.net/attachments/855797269190017074/901249988942766131/logo_Gamemode.png"		-- Icon top of the Embed
		...
		if IsEntityDead(PlayerPedId()) and not SendLogs then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'suicide' then
				...
				if debug then
					...
					ShowDebug('Event '..exports.Tree:serveurConfig().Serveur.color..'1~s~\nPlayerID: '..exports.Tree:serveurConfig().Serveur.color..source.. '~s~\nKillerID: '..exports.Tree:serveurConfig().Serveur.color.._killer)
					ShowDebug('DeathReason: '..exports.Tree:serveurConfig().Serveur.color..DeathReason.. '~s~\nWeapon: '..exports.Tree:serveurConfig().Serveur.color.._Weapon)
				end
			else
				TriggerServerEvent('playerDied',2,PlayerId(),GetPlayerServerId(Killer),DeathReason,Weapon)
				if debug then
					if Weapon then _Weapon = Weapon else _Weapon = "N/A" end if Killer then _killer = Killer else _killer = "N/A" end
					ShowDebug('Event '..exports.Tree:serveurConfig().Serveur.color..'2~s~\nPlayerID: '..exports.Tree:serveurConfig().Serveur.color..PlayerId().. '~s~\nKillerID: '..exports.Tree:serveurConfig().Serveur.color..GetPlayerServerId(Killer))
					ShowDebug('DeathReason: '..exports.Tree:serveurConfig().Serveur.color..DeathReason.. '~s~\nWeapon: '..exports.Tree:serveurConfig().Serveur.color.._Weapon)
				end
			end
			Killer = nil
...
-- 			if debug then
-- 				ShowDebug('Weapon: '..exports.Tree:serveurConfig().Serveur.color..WeaponNames[tostring(GetSelectedPedWeapon(playerped))])
-- 			end
-- 			Wait(weaponLogDelay)
-- 		end
...
function ShowDebug(text)
	...
	AddTextComponentSubstringPlayerName(exports.Tree:serveurConfig().Serveur.color.."GameCore Debug:~s~\n"..text)
