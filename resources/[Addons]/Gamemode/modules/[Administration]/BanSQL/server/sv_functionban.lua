...
function cmdban(source, args)
	...
	if args[1] then		
		...
		if target and target > 0 then
			...
			if ping and ping > 0 then
				if duree and duree < 365 then
					...
					local sourceplayername = ""
						...
						if duree > 0 then
							TriggerClientEvent("esx:showNotification", source, (exports.Tree:serveurConfig().Serveur.color.."Administration\n~s~"..Text.youban .. targetplayername .. Text.during .. duree .. Text.forr .. ReasonForBan))
							...
							local webhookLink = exports.Tree:serveurConfig().Logs.BanPlayers
			...
			TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color.."test\n~s~"..Text.invalidid)
		end
	else
		TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color.."Administration\n~s~"..Text.cmdban)
	end
			...
			if data[1] then
				if #data > 1 then
					...
					['@ban_id']  = data[1].ban_id
						...
						local xPlayer = ESX.GetPlayerFromId(source)
						local webhookLink = exports.Tree:serveurConfig().Logs.UnBanPlayers
			...
			if target ~= "" then
				MySQL.Async.fetchAll('SELECT * FROM baninfo WHERE license = @license', 
				{
					...
					if data[1] then
						...
						TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color.."test 10\n~s~"..Text.invalidid)
					end
				end)
			else
				TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color.."Administration\n~s~"..Text.invalidname)
...
function banOff(source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,permanent)
	...
	MySQL.Async.fetchAll('SELECT * FROM banlist WHERE targetplayername like @playername', 
	{
		...
		if not data[1] then
			...
			local xPlayer = ESX.GetPlayerFromId(source)
			local webhookLink = exports.Tree:serveurConfig().Logs.BanOffline
