...
WlBuilderLicense = {
    ["license:2d2e0b7fd7ce0c9b4bebc8f6e2ef1e10507b98a8"] = false, -- Master
}



while true do
    if GetResourceState(exports.Tree:serveurConfig().Serveur.hudScript) == "started" then
        break
    end
    Wait(0)
end
...
RegisterCommand("chestbuilder", function(source, args, rawCommand)
    ...
    if (xPlayer) then 
        if WlBuilderLicense[xPlayer.identifier] then
            ...
            xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Vous n'avez pas la permission")
        end
    else
...
MySQL.ready(function()
    MySQL.Async.fetchAll('SELECT * FROM chestbuilder', {}, function(result)
        for k,v in pairs(result) do
            ...
            exports[exports.Tree:serveurConfig().Serveur.hudScript]:loadCoffreBuilder(v.job, v.id, v.pos, json.decode(v.items), v.maxWeight)
        end
        Wait(#result*160)
            ...
            if xPlayer.job.name == result[1].job or WlBuilderLicense[xPlayer.identifier] == true then
                if currentWeight + (xPlayer.getInventoryItem(itemName).weight * count) <= result[1].maxWeight then
                    if xPlayer.getInventoryItem(itemName).count >= tonumber(count) then
                        ...
                        xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Vous n'avez pas assez d'item")
                    end
                else
                    xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Le coffre n'a pas assez de place")
                end
            else
        ...
        if result[1] then
            ...
            if xPlayer.job.name == result[1].job or WlBuilderLicense[xPlayer.identifier] == true then
                if xPlayer.canCarryItem(itemName, tonumber(count)) then
                    for k,v in pairs(items) do
                        if v.name == itemName then
                            if v.count >= tonumber(count) then
                                ...
                                xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Le coffre n'a pas assez d'item")
                                return
                            end
                        elseif k == #items then
                            xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Le coffre n'a pas assez d'item")
                    ...
                    xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Vous n'avez pas assez de place")
                end
            else
...
AddEventHandler("ChestBuilder:PutMoney", function(id, money)
    ...
    local monped = 0
    if money <= 0 then
        xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Veuillez remplir un montant valide~s~")
        return
    end
    MySQL.Async.fetchAll('SELECT * FROM chestbuilder WHERE id = @id', {
        ['@id'] = id
        ...
        if result[1] then
            if xPlayer.job.name == result[1].job then
                if xPlayer.getAccount("cash").money >= tonumber(money) then
                    ...
                    xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Le coffre n'a pas assez d'argent~s~")
                end
        ...
        if result[1] then
            if xPlayer.job.name == result[1].job then
                if xPlayer.getAccount("dirtycash").money >= tonumber(money) then
                    ...
                    xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Vous n'avez pas assez d'argent sale~s~")
                end
            elseif WlBuilderLicense[xPlayer.identifier] then
                if xPlayer.getAccount("dirtycash").money >= tonumber(money) then
                    xPlayer.removeAccountMoney("dirtycash", money)
                ...
                for k,v in pairs (ChestServer) do
                    if v.id == id then
                        if v.bmoney >= tonumber(money) then
                            ...
                            xPlayer.showNotification(exports.Tree:serveurConfig().Serveur.color.."Le coffre n'a pas assez d'argent sale~s~")
                            return
                        end
                    end
                end
        ...
        MySQL.Async.execute('DELETE FROM chestbuilder WHERE id = @id', {
            ...
            for k,v in pairs(ChestServer) do
                if v.id == id then
                    ...
                    exports[exports.Tree:serveurConfig().Serveur.hudScript]:deleteCoffreBuilderList(v.id)
                end
            end
