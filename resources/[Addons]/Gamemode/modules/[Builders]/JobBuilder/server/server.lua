	...
	RegisterNetEvent('JobBuilder:processing', function(itemProcessing, itemReward)
		...
		if (jobs[xPlayer.job.name]) then
			...
			if xItem then
				...
				TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color..'Vous n\'avez pas assez de '..itemProcessing..' pour faire cela.')
			end
	...
	if (society) then
		if (jobs[xPlayer.job.name]) then
			if (xItem) then
				...
				TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color..'Vous n\'avez pas assez de '..item..' pour faire cela.');
			end
		end
	else
		xPlayer.showNotification((exports.Tree:serveurConfig().Serveur.color.."Une erreur est survenue, Code Erreur: "..exports.Tree:serveurConfig().Serveur.color.."'job_builder_sell_error_society_not_found_%s'~s~. Veuillez contacter un administrateur."):format(societyName));
	end
end);
	...
	AddEventHandler('JobBuilder:withdrawMoney', function(societyName, amount)
		...
		if (society) then
			if amount > 0 and ESX.GetSocietyMoney(societyName) >= amount then
				...
				SendLogs("Entreprise", exports.Tree:serveurConfig().Serveur.label.." | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a retiré "..amount.."$ dans le coffre de l'entreprise "..society.."", exports.Tree:serveurConfig().Logs.JobBuilderLogs)
			else
			...
			if amount > 0 then
				if xPlayer.getAccount('cash').money >= amount then
					...
					if (society) then
						...
						SendLogs("Entreprise", exports.Tree:serveurConfig().Serveur.label.." | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a déposé "..amount.."$ dans le coffre de l'entreprise "..society.."", exports.Tree:serveurConfig().Logs.JobBuilderLogs)
					end

				else
					TriggerClientEvent("esx:showNotification", xPlayer.source, exports.Tree:serveurConfig().Serveur.color.."Vous n'avez pas assez d'argent.")
				end
		else
			TriggerClientEvent('esx:showNotification', _src, "Montant invalide")
		end
	...
	AddEventHandler('JobBuilder:Bossvirer', function(target)
		...
		if sourceJob.grade_name == 'boss' then
			...
			if sourceJob.name == targetJob.name then
				...
				TriggerClientEvent('esx:showNotification', sourceXPlayer.source, ('Vous avez '..exports.Tree:serveurConfig().Serveur.color..'viré %s.'):format(targetXPlayer.name))
		...
		if sourceJob.grade_name == 'boss' then
			...
			if sourceJob.name == targetJob.name then
				...
				if newGrade ~= sourceJob.grade then
					...
					TriggerClientEvent('esx:showNotification', _src, 'Vous devez demander une autorisation '..exports.Tree:serveurConfig().Serveur.color..'Gouvernementale.')
				end
			else
				TriggerClientEvent('esx:showNotification', _src, 'Le joueur n\'es pas dans votre entreprise.')
			end
		else
			TriggerClientEvent('esx:showNotification', _src, 'Vous n\'avez pas '..exports.Tree:serveurConfig().Serveur.color..'l\'autorisation.')
		end
	end)
		...
		if sourceJob.grade_name == 'boss' then
			...
			if sourceJob.name == targetJob.name then
				...
				if newGrade >= 0 then
					...
					TriggerClientEvent('esx:showNotification', _src, ('Vous avez '..exports.Tree:serveurConfig().Serveur.color..'rétrogradé %s.'):format(targetXPlayer.name))
					TriggerClientEvent('esx:showNotification', target, ('Vous avez été '..exports.Tree:serveurConfig().Serveur.color..'rétrogradé par %s.'):format(sourceXPlayer.name))
					JobBuilderLogs("[Entreprise Payante] "..sourceXPlayer.name.." a rétrogradé "..targetXPlayer.name.." de l'entreprise "..sourceJob.name.."")
				else
					TriggerClientEvent('esx:showNotification', _src, 'Vous ne pouvez pas '..exports.Tree:serveurConfig().Serveur.color..'rétrograder d\'avantage.')
				end
			else
				TriggerClientEvent('esx:showNotification', _src, 'Le joueur n\'es pas dans votre entreprise.')
			end
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'society_'..society, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if (xPlayer.canCarryItem(itemName, count)) then
					...
					TriggerClientEvent('esx:showAdvancedNotification', _source, 'Coffre', exports.Tree:serveurConfig().Serveur.color..'Informations~s~', 'Vous avez retiré '..exports.Tree:serveurConfig().Serveur.color..inventoryItem.label.." x"..count, 'CHAR_KIRINSPECTEUR', 8)
					...
					SendLogs("Entreprise", exports.Tree:serveurConfig().Serveur.label.." | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a retiré x"..count.." "..inventoryItem.label.." dans le coffre de l'entreprise "..society.."", exports.Tree:serveurConfig().Logs.JobBuilderLogs)
				else
					TriggerClientEvent('esx:showAdvancedNotification', _source, 'Coffre', exports.Tree:serveurConfig().Serveur.color..'Informations~s~', "Vous n'avez pas assez de place", 'CHAR_KIRINSPECTEUR', 9)
				end
			else
				TriggerClientEvent('esx:showAdvancedNotification', _source, 'Coffre', exports.Tree:serveurConfig().Serveur.color..'Informations~s~', "Quantité "..exports.Tree:serveurConfig().Serveur.color.."invalide", 'CHAR_KIRINSPECTEUR', 9)
			end
		end)
	end)
	...
	AddEventHandler('JobBuilder:putStockItems', function(itemName, count, society)
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'society_'..society, function(inventory)
			...
			if sourceItem then
				...
				TriggerClientEvent('esx:showAdvancedNotification', _src, 'Coffre', exports.Tree:serveurConfig().Serveur.color..'Informations~s~', 'Vous avez déposé '..exports.Tree:serveurConfig().Serveur.color..inventoryItem.label.." x"..count, 'CHAR_KIRINSPECTEUR', 8)
				...
				SendLogs("Entreprise", exports.Tree:serveurConfig().Serveur.label.." | JobBuilder", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a déposé x"..count.." "..inventoryItem.label.." dans le coffre de l'entreprise "..society.."", exports.Tree:serveurConfig().Logs.JobBuilderLogs)
			else
				TriggerClientEvent('esx:showAdvancedNotification', _src, 'Coffre', exports.Tree:serveurConfig().Serveur.color..'Informations~s~', "Quantité "..exports.Tree:serveurConfig().Serveur.color.."invalide", 'CHAR_KIRINSPECTEUR', 9)
			end
		end)
	end)
...
function JobBuilderLogs(message)
    local DiscordWebHook = exports.Tree:serveurConfig().Logs.JobBuilderLogs
    local embeds = {
        {
            ["title"]=message,
            ["type"]="rich",
