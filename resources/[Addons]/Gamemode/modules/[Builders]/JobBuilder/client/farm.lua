    ...
    while true do
        ...
        for k,v in pairs(jobsData) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == v.Name then
                ...
                if dist <= 3.0 then
                    Timer = 0
                    ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour récolter "..exports.Tree:serveurConfig().Serveur.color..v.labelitemR.."")
    ...
    while true do
        ...
        for k,v in pairs(jobsData) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == v.Name then
                ...
                if dist <= 3.0 then
                    Timer = 0
                    ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour traiter "..exports.Tree:serveurConfig().Serveur.color..v.labelitemR.."")
                    if IsControlJustPressed(1,51) then
        ...
        if (type(jobsData) == 'table') then
            for k,v in pairs(jobsData) do
                if ESX.PlayerData.job and ESX.PlayerData.job.name == v.Name then
                    ...
                    if dist <= 3.0 then
                        Timer = 0
                        ESX.ShowHelpNotification("Appuyer sur ~INPUT_PICKUP~ pour vendre "..exports.Tree:serveurConfig().Serveur.color..v.labelitemT.."")
                        if IsControlJustPressed(1,51) then
