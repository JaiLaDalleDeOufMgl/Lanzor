...
function GarageMenu()
    local MenuG = RageUIv1.CreateMenu("", JobBuilder.Garage.Label)

      RageUIv1.Visible(MenuG, not RageUIv1.Visible(MenuG))

          while MenuG do

              Citizen.Wait(0)

                  RageUIv1.IsVisible(MenuG, true, true, true, function()

                      RageUIv1.Button("Ranger la voiture", nil, {RightLabel = "→"}, function(Hovered, Active, Selected)
                          if (Selected) then
                      ...
                      RageUIv1.Separator(exports.Tree:serveurConfig().Serveur.color.."↓ Véhicule disponible ↓")


                    for k,v in pairs(json.decode(JobBuilder.Garage.vehInGarage)) do
                      RageUIv1.Button(v.label, nil, {RightLabel = "→"},true, function(Hovered, Active, Selected)
                          if (Selected) then
              ...
              if not RageUIv1.Visible(MenuG) then
