...
local actions = {
    ...
    ["Transform"] = function(source, drug, xPlayer)
        ...
        if actualCount then
            if actualCount.count < requieredCount then
...
AddEventHandler("exedrugs_deletedrug", function(drugID)
    ...
    if not drugs[drugID] then
        TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color.."Cette drogue n'existe plus")
        return
    end
...
AddEventHandler("exedrugs_create", function(builderInfos)
    ...
    MySQL.Async.fetchAll("SELECT * FROM items", {}, function(items)
        ...
        --     TriggerClientEvent("esx:showNotification", source, exports.Tree:serveurConfig().Serveur.color.."Un des item du drugsbuilder est invalide, création abandonnée")
        --     return
        -- end
