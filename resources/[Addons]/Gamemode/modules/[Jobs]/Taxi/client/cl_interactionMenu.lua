        ...
        RageUI.IsVisible(mainMenu, function()
            if ServiceTaxi then
                RageUI.Button("Envoyer une facture", nil, {
                    ...
                    onSelected = function()
                        local amount = KeyboardInputTaxi("Montant:", 'Indiquez un montant', '', 7)
                        if tonumber(amount) == nil then
                            ...
                            if closestPlayer == -1 or closestDistance > 3.0 then
                                ...
                                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'taxi', 'Taxi', tonumber(amount))
                            end
                        end
                    end
                });
        ...
        if ESX.PlayerData.job.name == 'taxi' then
            for k,v in pairs(Config.Jobs.Taxi.Clothes) do
                if #(coords - v.clothes) <= 10 then
                    DrawMarker(20, v.clothes, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], Config.Get.Marker.Color[1], Config.Get.Marker.Color[2], Config.Get.Marker.Color[3], 255, 28, 112, 0, 0, nil, nil, 0)
                    interval = 1
...
function KeyboardInputTaxi(entryTitle, textEntry, inputText, maxLength)
