    ...
    while mainMenu do
        RageUI.IsVisible(mainMenu, function()
            ...
            if ServiceMecano then
                ...
                RageUI.Button("Annonce "..exports.Tree:serveurConfig().Serveur.color.."[Fermetures]"..exports.Tree:serveurConfig().Serveur.color.."", nil, {}, true, {
                    onSelected = function()
                    TriggerServerEvent('Ferme:Mecano')
                    end
                })
                RageUI.Button("Annonce "..exports.Tree:serveurConfig().Serveur.color.."[Recrutement]", nil, {}, true, {
                    onSelected = function()
                ...
                RageUI.Separator(exports.Tree:serveurConfig().Serveur.color.."Vous devez étre en service") 
            end       
        end)
...
AddEventHandler('use:jerican', function()
    ...
    if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
        ...
        if DoesEntityExist(vehicle) then
            TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
            Citizen.CreateThread(function()
                local fuel = exports[exports.Tree:serveurConfig().Serveur.hudScript]:GetFuel(vehicle)
...
local LsCustoms = {
    ...
    Pos = {
        {pos = vector3(-219.9385, -1342.9546, 31.4133), job = "mecano", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-219.9478, -1337.4071, 31.4223), job = "mecano", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-202.9828, -1340.8479, 31.3996), job = "mecano", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-202.9216, -1336.7473, 31.3996), job = "mecano", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-333.0874, -132.7575, 39.0096), job = "mecano2", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-312.5800, -125.0531, 39.1688), job = "mecano2", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-313.1994, -134.7802, 39.0097), job = "mecano2", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-317.1739, -145.2002, 39.0115), job = "mecano2", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
        {pos = vector3(-320.9934, -156.2785, 39.0097), job = "mecano2", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au "..exports.Tree:serveurConfig().Serveur.color.."Benny's~s~.", size = 90.0},
    },
...
RegisterNetEvent("JustGod:mechanic:receivePlayerBillsState", function(bool)
    ...
    if (bool ~= false) then
        ...
        if (player ~= -1 and distance <= 3.0) then
            ...
            TriggerServerEvent('babyboy:sendBilling', GetPlayerServerId(player), FactureJob, math.floor(LsCustoms.Price*3.0))
            ...
            ResetAll();
            TriggerEvent('esx:showAdvancedNotification', 'mecano', exports.Tree:serveurConfig().Serveur.color..'mecano', exports.Tree:serveurConfig().Serveur.color..'Personne autour. ~s~Tous les changements ont été annulés.', 'CHAR_LS_CUSTOMS', 2);
            FactureJob = nil;
            banana = nil;
        ...
        SetVehicleProperties(LsCustoms.Vehicle, LsCustoms.Prop);
        ResetAll();
        TriggerEvent('esx:showAdvancedNotification', 'mecano', exports.Tree:serveurConfig().Serveur.color..'mecano', 'La personne sélectionner a déjé une ou plusieurs factures impayées.', 'CHAR_LS_CUSTOMS', 2);
        FactureJob = nil;
        banana = nil;
                ...
                while LsCustoms.openedMenu do
                    ...
                    if (player ~= -1 and distance <= 5.0) then
                        ...
                        if LsCustoms.Price > 0 then 
                            --DrawTextPrice("Total facture : "..exports.Tree:serveurConfig().Serveur.color..""..math.floor(LsCustoms.Price*3.0).."$~s~. Total entreprise : ~g~"..LsCustoms.Price.."$~s~.")
                        end
                        ...
                        RageUIv4.IsVisible(LsCustoms.subMenuModification, function()
                            if LsCustoms.Price > 0 then
                                RageUIv4.Button("Valider les modifications", nil, {RightLabel = "~g~"..LsCustoms.Price.."$"}, true, {
                                    onSelected = function()
                                        local player, distance = ESX.Game.GetClosestPlayer();
                                        TriggerServerEvent("JustGod:mechanic:requestPlayerBillsState", GetPlayerServerId(player), LsCustoms.Price);
                                    end
                                })
                            end
                            RageUIv4.Button("Annuler les modifications", nil, {RightLabel = "→"}, true, {
                                onSelected = function()
                                    ...
                                    SetVehicleProperties(LsCustoms.Vehicle, LsCustoms.Prop)
                                    ResetAll()
                                    ESX.ShowNotification("Vous avez "..exports.Tree:serveurConfig().Serveur.color.."annuler~s~ toutes "..exports.Tree:serveurConfig().Serveur.color.."les modifications~s~.")
                                    FactureJob = nil
                                    banana = nil
                                end
                            })
...
AddEventHandler('CancelLsCustoms', function()
	ESX.ShowNotification("La "..exports.Tree:serveurConfig().Serveur.color.."commande~s~ a été "..exports.Tree:serveurConfig().Serveur.color.."refusé~s~ car l'entreprise n'a plus assez de "..exports.Tree:serveurConfig().Serveur.color.."fonds nécessaires~s~.")
	SetVehicleProperties(LsCustoms.Vehicle, LsCustoms.Prop)
end)
        ...
        if ESX.PlayerData.job.name == 'mecano' then
            for k,v in pairs(Config.Jobs.Mecano.ShopMec) do
                if #(coords - v.shopmec) <= 10 then
                    DrawMarker(Config.Get.Marker.Type, v.shopmec, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], Config.Get.Marker.Color[1], Config.Get.Marker.Color[2], Config.Get.Marker.Color[3], 170, 0, 1, 0, 0, nil, nil, 0)
                    interval = 1
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if ESX.PlayerData.job.name == 'mecano2' then
            for k,v in pairs(Config.Jobs.LsCustom.ShopMec) do
                if #(coords - v.shopmec) <= 10 then
                    DrawMarker(Config.Get.Marker.Type, v.shopmec, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], Config.Get.Marker.Color[1], Config.Get.Marker.Color[2], Config.Get.Marker.Color[3], 170, 0, 1, 0, 0, nil, nil, 0)
                    interval = 1
                    if #(coords - v.shopmec) <= 3 then
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local coords = GetEntityCoords(plyPed)
        if ESX.PlayerData.job.name == 'mecano2' then
            for k,v in pairs(Config.Jobs.LsCustom.Clothes) do
                if #(coords - v.clothes) <= 10 then
                    ...
                    if #(coords - v.clothes) <= 3 then
                        Citizen.CreateThread(function()
                            ...
                            if IsControlJustPressed(0, 51) then
                                CreateThread(function()
                                    OpenClotheForCustom = true
                                    OpenLscustomClothesMenu()
                                end)
                            end
                        end)
                    end
    ...
    while true do
        ...
        if ESX.PlayerData.job.name == 'mecano2' then
            if #(coords - vector3(-352.0392, -108.8868, 38.7004)) <= 10 then
                DrawMarker(Config.Get.Marker.Type, vector3(-352.0392, -108.8868, 38.7004), 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], Config.Get.Marker.Color[1], Config.Get.Marker.Color[2], Config.Get.Marker.Color[3], 170, 0, 1, 0, 0, nil, nil, 0)
                interval = 1
                if #(coords - vector3(-352.0392, -108.8868, 38.7004)) <= 3 then
                    Citizen.CreateThread(function()
                        ...
                        if not codesCooldown then
                            ESX.Game.SpawnVehicle('towtruck', vector3(-383.82, -125.52, 38.68), 301, function (vehicle)
                                local newPlate = exports['Gamemode']:GeneratePlate()
                                SetVehicleNumberPlateText(vehicle, newPlate)
                                exports[exports.Tree:serveurConfig().Serveur.hudScript]:SetFuel(vehicle, 100)
                                TriggerServerEvent('babyboy:GiveDoubleKeys', 'yes', newPlate)
...
function OpenMenuMecanoJob()
    if open then
        ...
        CreateThread(function()
            while open do 
                local PosMenuMecano = vector3(-180.98, -1283.63, 31.29)
                ...
                RageUI.IsVisible(MecanoJobGarage,function() 
                    ...
                    if ESX.PlayerData.job.grade_name == 'boss' then
                        RageUI.Button("Tailgater Patron", nil, {RightLabel = "→→"}, true , {
                            onSelected = function()
                                TriggerServerEvent('mecano:spawnVehicleMecano')
                                ESX.Game.SpawnVehicle('tailgater2', vector3(-184.24, -1293.31, 31.29), 156.79, function (vehicle)
                                    local newPlate = exports['Gamemode']:GeneratePlate()
                                    SetVehicleNumberPlateText(vehicle, newPlate)
                                    exports[exports.Tree:serveurConfig().Serveur.hudScript]:SetFuel(vehicle, 100)
                    ...
                    RageUI.Button("Dépaneuse", nil, {RightLabel = "→→"}, true , {
                        onSelected = function()
                            TriggerServerEvent('mecano:spawnVehicleMecano')
                            ESX.Game.SpawnVehicle('towtruck', vector3(-184.24, -1293.31, 31.29), 156.79, function (vehicle)
                            local newPlate = exports['Gamemode']:GeneratePlate()
                            SetVehicleNumberPlateText(vehicle, newPlate)
                            exports[exports.Tree:serveurConfig().Serveur.hudScript]:SetFuel(vehicle, 100)
                            TriggerServerEvent('babyboy:GiveDoubleKeys', 'no', newPlate)
                    ...
                    RageUI.Button("Camion de service", nil, {RightLabel = "→→"}, true , {
                        onSelected = function()
                            TriggerServerEvent('mecano:spawnVehicleMecano')
                                ESX.Game.SpawnVehicle('speedo4', vector3(-184.24, -1293.31, 31.29), 156.79, function (vehicle)
                                local newPlate = exports['Gamemode']:GeneratePlate()
                                SetVehicleNumberPlateText(vehicle, newPlate)
                                exports[exports.Tree:serveurConfig().Serveur.hudScript]:SetFuel(vehicle, 100)
                                TriggerServerEvent('babyboy:GiveDoubleKeys', 'no', newPlate)
                    ...
                    RageUI.Button("4x4", nil, {RightLabel = "→→"}, true , {
                        onSelected = function()
                            TriggerServerEvent('mecano:spawnVehicleMecano')
                                ESX.Game.SpawnVehicle('trophytruck', vector3(-184.24, -1293.31, 31.29), 156.79, function (vehicle)
                                local newPlate = exports['Gamemode']:GeneratePlate()
                                SetVehicleNumberPlateText(vehicle, newPlate)
                                exports[exports.Tree:serveurConfig().Serveur.hudScript]:SetFuel(vehicle, 100)
                                TriggerServerEvent('babyboy:GiveDoubleKeys', 'no', newPlate)
        ...
        for k in pairs(position) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mecano' then 
                ...
                if dist <= 15.0 then
                    wait = 0
                    if ServiceMecano then
                        DrawMarker(Config.Get.Marker.Type, -180.98, -1283.63, 31.29, 0, 0, 0, Config.Get.Marker.Rotation, nil, nil, Config.Get.Marker.Size[1], Config.Get.Marker.Size[2], Config.Get.Marker.Size[3], Config.Get.Marker.Color[1], Config.Get.Marker.Color[2], Config.Get.Marker.Color[3], 170, 0, 1, 0, 0, nil, nil, 0)
                        if dist <= 1.0 then
                            Citizen.CreateThread(function()
