...
local function StopDriveTest(success)
	if success then
		...
		TriggerServerEvent('GamemodePass:taskCountAdd:standart', 1, 1)
...
local CheckPoints = {

	{
		...
		Action = function(playerPed, setCurrentZoneType)
			ESX.ShowNotification("Allons sur la route, tournez à droite, vitesse limitée à "..exports.Tree:serveurConfig().Serveur.color.."80km/h")
...

	{
		...
		Action = function(playerPed, setCurrentZoneType)
			ESX.ShowNotification("Vous entrez sur l'autoroute, vitesse limitée à "..exports.Tree:serveurConfig().Serveur.color.."120km/h")
 ...
 --
	{
		...
		Action = function(playerPed, setCurrentZoneType)
			ESX.ShowNotification("Prenez à droite, vitesse limité à "..exports.Tree:serveurConfig().Serveur.color.."80km/h")
			TesOuFDPRendMonQuad = 1
		end
	},
		...
		if CurrentTest == 'drive' then
			...
			if IsPedInAnyVehicle(PlayerPedId(), false) then
				...
				if TesOuFDPRendMonQuad == 0 and GetSpeed >= speed_limit_residence then
					...
					if DriveErrors <= 5 then
						ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color.."Vous avez fait une erreur Vous roulez trop vite, vitesse limite : " ..speed_limit_residence.. " km/h!\n"..exports.Tree:serveurConfig().Serveur.color.."Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000) -- evite bug
				end
				...
				if TesOuFDPRendMonQuad == 1 and GetSpeed >= speed_limit_ville then
					...
					IsAboveSpeedLimit = true
					if DriveErrors <= 5 then
						ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color.."Vous avez fait une erreur Vous roulez trop vite, vitesse limite : " ..speed_limit_ville.. " km/h!\n"..exports.Tree:serveurConfig().Serveur.color.."Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000)
				end
				...
				if TesOuFDPRendMonQuad == 2 and GetSpeed >= speed_limit_otoroute then
					...
					IsAboveSpeedLimit = true
					if DriveErrors <= 5 then
						ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color.."Vous avez fait une erreur Vous roulez trop vite, vitesse limite : " ..speed_limit_otoroute.. " km/h!\n"..exports.Tree:serveurConfig().Serveur.color.."Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000)
				end
				...
				if HasEntityCollidedWithAnything(vehicle) and DamageControl == 0 then
					DriveErrors       = DriveErrors + 1
					if DriveErrors <= 5 then
						ESX.ShowNotification(exports.Tree:serveurConfig().Serveur.color.."Vous avez fait une erreur Votre vehicule s\'est pris des dégats\n"..exports.Tree:serveurConfig().Serveur.color.."Nombre d'erreurs " ..DriveErrors.. "/5")
					end
					Wait(2000)
				end
...
RMenuv1.Add('rz-permis', 'main', RageUIv1.CreateMenu("", exports.Tree:serveurConfig().Serveur.color.."Bienvenue à l\'auto-ecole"))
        ...
        Citizen.CreateThread(function()
			while AutoEcoleS.Menu do
				RageUIv1.IsVisible(RMenuv1:Get('rz-permis', 'main'), true, true, true, function()
					...
					RageUIv1.ButtonWithStyle("Passer le permis", "Passer le permis pour "..exports.Tree:serveurConfig().Serveur.color.."1200$.", { RightLabel = "1200$" }, true, function(_, _, Selected)
						if Selected then
							StartDriveTest()
