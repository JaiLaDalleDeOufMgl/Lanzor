...
AddEventHandler("splayer_staff_event:GetInfo", function(startPos, endPos, desc, NombreJoueurs, veh, prix, ArgentPropre)
    ...
    if not EventEnCourds then
        ...
        if ArgentPropre then
            ESX.ShowNotification("~o~INFORMATION\n~s~Participant Max: "..exports.Tree:serveurConfig().Serveur.color..NombreJoueurs.."\n~s~Type de véhicule: "..exports.Tree:serveurConfig().Serveur.color..veh.."\n~s~Gain: "..exports.Tree:serveurConfig().Serveur.color..prix.."~s~$\nType de gain: ~o~Argent propre.")
        else
        ...
        while not started do
            ...
            if dst <= 3.0 then
                ESX.ShowNotification("Appuyer sur "..exports.Tree:serveurConfig().Serveur.color.."[E]~s~ pour lancer la mission")
                if IsControlJustReleased(1, 38) and not pressed and not IsPedInAnyVehicle(pPed, false) and ESX.Game.IsSpawnPointClear(startPos, 5.0) then
                    ...
                    ESX.ShowNotification("Tu dois être à pied pour lancer la mission, appuie sûr "..exports.Tree:serveurConfig().Serveur.color.."E~s~ pour lancer la mission\n~o~La zone doit être dégagé pour sortir le véhicule d'event !")
                end
            end
        end
    end
    ...
    while not finish do
        ...
        if dst <= 50.0 then
            ...
            if dst <= 3.0 then
                ...
                ESX.ShowNotification("Event terminé\nTu à gagné: "..exports.Tree:serveurConfig().Serveur.color..prix.."~s~$ cash")
