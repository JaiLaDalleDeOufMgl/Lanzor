...
function RageUI.StatisticPanelString(Title, Text, Index)
    local CurrentMenu = RageUI.CurrentMenu
    if CurrentMenu ~= nil then
        if CurrentMenu() and (Index == nil or (CurrentMenu.Index == Index)) then

            RenderRectangle(CurrentMenu.X, CurrentMenu.Y + Statistics.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + (RageUI.StatisticPanelCount * 42), Statistics.Background.Width + CurrentMenu.WidthOffset, Statistics.Background.Height, 0, 0, 0, 170)
            RenderText(Title or "", CurrentMenu.X + 8.0, (RageUI.StatisticPanelCount * 40) + CurrentMenu.Y + Statistics.Text.Left.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset - 5, 0, Statistics.Text.Left.Scale, 245, 245, 245, 255, 0)
            RenderText(Text or "", CurrentMenu.X + CurrentMenu.SafeZoneSize.X + (CurrentMenu.WidthOffset / 2) + 200, (RageUI.StatisticPanelCount * 40) + CurrentMenu.Y + Statistics.Text.Left.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset - 5, 0, Statistics.Text.Left.Scale, 245, 245, 245, 255, 0)
            RageUI.StatisticPanelCount = RageUI.StatisticPanelCount + 1
        end
    end
end
