window.APP = {
	...
	data() {
		return {
			...
			removedSuggestions: [],
			templates: {
				'default': '<div class="chat-message">Gamemode {0}: {1}</div>',
				'defaultAlt': '{0}',
				'print': '<pre>{0}</pre>',
	...
	mounted() {
		post('http://GalacticChat/loaded', JSON.stringify({}));
	...
	methods: {
		...
		send(e) {
			if (this.message !== '') {
				post('http://GalacticChat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('http://GalacticChat/chatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
